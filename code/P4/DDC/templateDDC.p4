...
control MyIngress(inout headers hdr,
	...
	register<bit<1>>({{portsCount}}) direction_r;
	register<bit<1>>({{portsCount}}) local_seq_r;
	register<bit<1>>({{portsCount}}) remote_seq_r;


	//port status
	register<bit<1>>({{portsCount}}) port_status_r;
    ...
    apply {
		...
		{% for p in ports %}
            bit<1> dir_{{p}};
            bit<1> port_status_{{p}};
            direction_r.read(dir_{{p}}, {{p}});
            port_status_r.read(port_status_{{p}},{{p}});
		{% endfor %}
            ...
            else
            {
                ...
                {% for p in ports[1:] %}
                    direction_r.read(dir_n, {{p}});           
                    if(dir_n==0 && standard_metadata.ingress_port != 1 || port_status_{{p}} ==0 ){
                    {% endfor %}
                    {% for p in ports %}
                        reverse_in_to_out({{p}});
                    {% endfor %}
                ...
                if(direction==1){
                    ...
                    {% endfor %}
                        ...
                        // and send it from one of the new OUT links
                            ...
                            {% for p in ports[1:] %}                       
                                   else if (dir_{{p}} == 1 && port_status_{{p}} ==1 ) {
                                        //hdr.myTunnel.packet_seq=1;
                                        send_on_outlink({{p}});
                                    }
                            {% endfor %}
