...
def updatePathsGraph(path,req):
    ...
    while i< len(path)-1:
        ...
        if isinstance(G[node1][node2][0]['bandwidth'], int):
            bw=G[node1][node2][0]['bandwidth']
        else:
            bw=G[node1][node2][0]['bandwidth'][0]
        bw-=req
        G[node1][node2][0]['bandwidth']=bw
...
def writeRegisterRules(h):
    fname="s%d-commands.txt"%(int(h)+1)
    f1 = open(fname, "a")
    ruleReg="register_write MyIngress.port_reg 0 31 \n"
...
def YenKSP(Graph, source, target, K, weight):
    ...
    def path2length(path):
        return sum(Graph[i][j][0][weight] for i, j in zip(path, path[1:]))
...
H = nx.DiGraph()
H= nx.read_graphml('AttMpls.graphml')
...
for e in bEdges:
    ...
    distance = math.acos(first_product + (second_product_first_part * second_product_second_part)) * 6378.137
    latency = ( distance * 1000 ) / ( 197000 )
    H[e[0]][e[1]][0]['latency']=latency
    H[e[0]][e[1]][0]['bandwidth']=45
...
while k<1:
    ...
    while itr<15:
        ...
        '''        
        elif pathLatency<bestBackupLatency and (set(currentBestBackupPath).issubset(set(currentBestPath))) and itr>1 :
            currentBestBackupPath[:]=[]
            bestBackupLatency=pathLatency
            for i in path:
                print i.name," ",
    ...
    #print("the number of paths is ",len(listOfLists))
    #for pp in listOfLists:
    #    for h in pp:
    #        print h.name,
    #    print()
    BBPath=[]
